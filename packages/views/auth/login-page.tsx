...
interface DingTalkAuthConfig {
  clientId: string;
  redirectUri: string;
  /** Opaque state passed through DingTalk OAuth (e.g. "platform:desktop"). */
  state?: string;
}
...
interface LoginPageProps {
   ...
   *  would be absurd). */
  extra?: ReactNode;
  /** When true, hides the email input and shows only OAuth buttons. */
  hideEmail?: boolean;
}
...
export function LoginPage({
  ...
  const oauthButtons = (
    <>
      ...
      {dingtalk && (
        <>
          {(!hideEmail || google || onGoogleLogin) && (
            <div className="relative w-full">
              <div className="absolute inset-0 flex items-center">
                <span className="w-full border-t" />
              </div>
