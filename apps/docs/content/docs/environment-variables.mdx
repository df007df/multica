...
The [Gitee PR ↔ issue integration](/gitee-integration) (gitee.com public cloud) uses a single shared secret. Each repository webhook must send the same value in `X-Gitee-Token`.

| Variable | Default | Description |
|---|---|---|
| `GITEE_WEBHOOK_SECRET` | empty | Password configured on each Gitee repo webhook. Must match the `X-Gitee-Token` header on every delivery |

**Behavior when unset:**

- **Settings → Gitee** shows a "not configured" hint.
- `POST /api/webhooks/gitee` returns **`503 gitee webhooks not configured`**.

See [Gitee integration → Self-host setup](/gitee-integration#self-host-setup) for per-repo webhook steps and repository URL registration.
