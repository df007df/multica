...
[gitee.com 公有云 PR ↔ issue 集成](/gitee-integration) 使用一个全局 secret；每个仓库的 WebHook 密码都要与之一致，并通过 `X-Gitee-Token` 请求头送出。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `GITEE_WEBHOOK_SECRET` | 空 | 与各 Gitee 仓库 WebHook 上配置的密码相同，用于校验 `X-Gitee-Token` |

**未配置时：**

- **Settings → Gitee** 显示未配置提示；
- `POST /api/webhooks/gitee` 返回 **`503 gitee webhooks not configured`**。

逐步配置见 [Gitee 集成 → Self-Host 配置](/gitee-integration#self-host-配置)。
