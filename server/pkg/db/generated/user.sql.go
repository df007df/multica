...
VALUES ($1, $2, $3, $4)
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, dingtalk_id
`
...
type CreateUserParams struct {
	...
	DingtalkID pgtype.Text `json:"dingtalk_id"`
}
...
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, dingtalk_id FROM "user"
WHERE id = $1
`
...
const getUserByDingTalkID = `-- name: GetUserByDingTalkID :one
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, dingtalk_id FROM "user"
WHERE dingtalk_id = $1
`

func (q *Queries) GetUserByDingTalkID(ctx context.Context, dingtalkID pgtype.Text) (User, error) {
	row := q.db.QueryRow(ctx, getUserByDingTalkID, dingtalkID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const linkDingTalkID = `-- name: LinkDingTalkID :one
UPDATE "user" SET dingtalk_id = $2, updated_at = now()
WHERE id = $1
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, dingtalk_id
`

type LinkDingTalkIDParams struct {
	ID         pgtype.UUID `json:"id"`
	DingtalkID pgtype.Text `json:"dingtalk_id"`
}

func (q *Queries) LinkDingTalkID(ctx context.Context, arg LinkDingTalkIDParams) (User, error) {
	row := q.db.QueryRow(ctx, linkDingTalkID, arg.ID, arg.DingtalkID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
