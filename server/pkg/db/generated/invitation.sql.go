...
RETURNING id, workspace_id, inviter_id, invitee_email, invitee_user_id, role, status, created_at, updated_at, expires_at, invite_type
`
...
func (q *Queries) CreateInvitationLink(ctx context.Context, arg CreateInvitationLinkParams) (WorkspaceInvitation, error) {
	row := q.db.QueryRow(ctx, createInvitationLink, arg.WorkspaceID, arg.InviterID, arg.Role)
	var i WorkspaceInvitation
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT id, workspace_id, inviter_id, invitee_email, invitee_user_id, role, status, created_at, updated_at, expires_at, invite_type FROM workspace_invitation
WHERE id = $1
`
...
SELECT wi.id, wi.workspace_id, wi.inviter_id, wi.invitee_email, wi.invitee_user_id, wi.role, wi.status, wi.created_at, wi.updated_at, wi.expires_at, wi.invite_type,
...
const listPendingLinkInvitationsByWorkspace = `-- name: ListPendingLinkInvitationsByWorkspace :many
SELECT wi.id, wi.workspace_id, wi.inviter_id, wi.invitee_email, wi.invitee_user_id, wi.role, wi.status, wi.created_at, wi.updated_at, wi.expires_at, wi.invite_type, u.name AS inviter_name, u.email AS inviter_email
FROM workspace_invitation wi
JOIN "user" u ON u.id = wi.inviter_id
...
type ListPendingLinkInvitationsByWorkspaceRow struct {
	ID            pgtype.UUID        `json:"id"`
	WorkspaceID   pgtype.UUID        `json:"workspace_id"`
	InviterID     pgtype.UUID        `json:"inviter_id"`
	InviteeEmail  pgtype.Text        `json:"invitee_email"`
...
func (q *Queries) ListPendingLinkInvitationsByWorkspace(ctx context.Context, workspaceID pgtype.UUID) ([]ListPendingLinkInvitationsByWorkspaceRow, error) {
	rows, err := q.db.Query(ctx, listPendingLinkInvitationsByWorkspace, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPendingLinkInvitationsByWorkspaceRow{}
	for rows.Next() {
		var i ListPendingLinkInvitationsByWorkspaceRow
		if err := rows.Scan(
			&i.ID,
			&i.WorkspaceID,
			&i.InviterID,
